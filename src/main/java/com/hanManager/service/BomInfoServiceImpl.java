...
public class BomInfoServiceImpl implements BomInfoService {
	...
	public List<HashMap<String, Object>> selectBomInfoCoatingList(HashMap<String, Object> params) throws Exception {
		...
		return bomInfoMapper.selectBomInfoCoatingList(params);
	}
	...
	public HashMap<String, Object> bomInfoCoatingSave(HashMap<String, Object> params) throws Exception {
		...
		if(ilist.size() > 0){
			for(int i=0; i<ilist.size();i++){
				ilist.get(i).put("CREATE_USER", params.get("loginUser"));
				ilist.get(i).put("UPDATE_USER", params.get("loginUser"));
				icnt += insertCoating(ilist.get(i));
			}
		}
		...
		if(ulist.size() > 0){
			for(int i=0; i<ulist.size();i++){
				ulist.get(i).put("CREATE_USER", params.get("loginUser"));
				ulist.get(i).put("UPDATE_USER", params.get("loginUser"));
				ucnt += updateCoating(ulist.get(i));
			}
		}
		...
		if(dlist.size() > 0){
			for(int  i=0; i<dlist.size();i++){
				dlist.get(i).put("CREATE_USER", params.get("loginUser"));
				dlist.get(i).put("UPDATE_USER", params.get("loginUser"));
				dcnt += deleteCoating(dlist.get(i));
			}
		}
	...
	public Integer insertCoating(HashMap<String, Object> params) throws Exception {
		try {
	        int retCode = 1;
	        bomInfoMapper.insertBomInfoCoating(params);
			return retCode;
		}catch (Exception e) {
			e.printStackTrace();
			throw new Exception(e.getMessage());
	...
	public Integer updateCoating(HashMap<String, Object> params) throws Exception {
		try {
			int retCode = 1;
	        bomInfoMapper.updateBomInfoCoating(params);
			return retCode;
		}catch (Exception e) {
			e.printStackTrace();
			throw new Exception(e.getMessage());
	...
	public Integer deleteCoating(HashMap<String, Object> params) throws Exception {
		try {
			int retCode = 1;
	        bomInfoMapper.deleteBomInfoCoating(params);
			return retCode;
		}catch (Exception e) {
			e.printStackTrace();
			throw new Exception(e.getMessage());
