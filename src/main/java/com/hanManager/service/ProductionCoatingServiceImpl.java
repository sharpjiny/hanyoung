...
@Service
public class ProductionCoatingServiceImpl implements ProductionCoatingService {
	...
	public List<HashMap<String, Object>> selectPrdtCoatingList(HashMap<String, Object> params) throws Exception {
		...
		return productionCoatingMapper.selectPrdtCoatingList(params);
	}
	...
	public HashMap<String, Object> prdtCoatingSave(HashMap<String, Object> params) throws Exception {
		...
		if(ilist.size() > 0){
			for(int i=0; i<ilist.size();i++){
				...
				ipCnt += insertPrdtStatusCoating(ilist.get(i));
		...
		if(ulist.size() > 0){
			for(int i=0; i<ulist.size();i++){
				...
				if("OK".equals(ulist.get(i).get("IS_CHECK"))){
					...
					upCnt += updatePrdtStatusCoating(ulist.get(i));
				}
			}
		...
		if(dlist.size() > 0){
			for(int  i=0; i<dlist.size();i++){
				...
				dpCnt += deletePrdtStatusCoating(dlist.get(i));
	...
	public Integer insertPrdtStatusCoating(HashMap<String, Object> params) throws Exception {
		try {
	        int retCode = 1;
	        productionCoatingMapper.insertPrdtStatusCoating(params);
	        ...
	        /*if("product02".equals(params.get("PRODUCT_GUBUN")) || "product05".equals(params.get("PRODUCT_GUBUN"))){
	        	if(params.get("PROD_ID") == null || "".equals(params.get("PROD_ID"))){
	        		...
	        		list = prdtCoatingMapper.selectPrdtCoatingId(params);
		        	params.put("PROD_ID", list.get(0).get("CODE"));
	        	}
	        	prdtCoatingMapper.mergePrdtStock(params);
	        }*/
			return retCode;
		}catch (Exception e) {
			e.printStackTrace();
	...
	public Integer updatePrdtStatusCoating(HashMap<String, Object> params) throws Exception {
		try {
			int retCode = 1;
			productionCoatingMapper.updatePrdtStatusCoating(params);
	...
	public Integer deletePrdtStatusCoating(HashMap<String, Object> params) throws Exception {
		try {
			int retCode = 1;
			productionCoatingMapper.deletePrdtStatusCoating(params);
	        
	        // 재고 테이블 update
	        /*if("product02".equals(params.get("PRODUCT_GUBUN")) || "product05".equals(params.get("PRODUCT_GUBUN"))){
	        	prdtCoatingMapper.deletePrdtStock(params);
	        }*/
			return retCode;
		}catch (Exception e) {
			e.printStackTrace();
	...
	public Integer insertPrdtMtrlsUsage(HashMap<String, Object> params) throws Exception {
		try {
	        int retCode = 1;
	        productionCoatingMapper.insertPrdtMtrlsUsage(params);
	...
	public Integer updatePrdtMtrlsUsage(HashMap<String, Object> params) throws Exception {
		try {
			int retCode = 1;
			productionCoatingMapper.updatePrdtMtrlsUsage(params);
	...
	public Integer deletePrdtMtrlsUsage(HashMap<String, Object> params) throws Exception {
		try {
			int retCode = 1;
			productionCoatingMapper.deletePrdtMtrlsUsage(params);
	...
	public Integer insertToMtrlsInOut(HashMap<String, Object> params) throws Exception {
		try {
	        ...
	        if(!"".equals(insertMap.get("THICKNESS_1")) && insertMap.get("THICKNESS_1") != null){
	        	...
	        	productionCoatingMapper.insertToMtrlsInOut(insertMap);
	        }
