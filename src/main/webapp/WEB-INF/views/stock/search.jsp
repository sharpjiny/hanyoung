                ...
                <table style="width:100%;" class="table-hover" >
                    ...
                    <c:choose>
						...
						<c:set var="loopIndex" value="${pageParam.startNum}"/>
							...
							<c:set var="totalPerBox" value="0"/>
							<c:set var="totalMoney" value="0"/>
							<c:set var="totalStockCnt" value="0"/>
							<c:forEach items="${list}" var="item" varStatus="loop">
								<c:set var="tmp" value="${item.stock_date}+${item.stock_time}+${item.car_num}+${item.work_type_name}+${item.stock_cnt}"/>
					            <tr>
					                <td class="center">
						                <c:choose>
						                <c:when test="${userSession.id == 'bogoadmin'}">
						                <a href="form?stockId=${item.stock_id}">${loopIndex+loop.count}</a>
						                </c:when>
						                <c:otherwise>
					                ...
					                <c:if test="${prevCode !=tmp}">
					                <td rowspan="${item.loop_cnt}" style="vertical-align: middle; text-align: center">${item.stock_date}</td>
					                <%-- <td class="center">${item.settlement_month}</td> --%>
					                <td rowspan="${item.loop_cnt}" style="vertical-align: middle; text-align: center">${item.stock_time}</td>
					                <td rowspan="${item.loop_cnt}" style="vertical-align: middle; text-align: center">${item.stock_cnt}</td>
					                <td rowspan="${item.loop_cnt}" style="vertical-align: middle; text-align: center">${item.car_num}</td>
					                <td rowspan="${item.loop_cnt}" style="vertical-align: middle; text-align: center">${item.work_type_name}</td>
					                <c:set var="totalStockCnt" value="${totalStockCnt + item.stock_cnt}"/>
					                </c:if>
					                ...
					                <td class="center"><fmt:formatNumber value="${item.box_num}" pattern="#,###" /></td>
					                <td class="center"><fmt:formatNumber value="${item.per_box_num}" pattern="#,###" /></td>
					                <td class="center">${item.price}</td>
					                <td class="center"><fmt:formatNumber value="${item.quantity2}" pattern="#,###.00" /></td>
					                <td class="center">
                                    <c:if test="${item.money ne 0}"><fmt:formatNumber value="${item.money}" pattern="#,###.00" /></c:if>
                                    <c:if test="${item.money eq 0}">0</c:if>
                                    </td>
					                <td class="center">${item.in_out_man}</td>
					                <td class="center">
					                <c:choose>
					            ...
					            <c:set var="prevCode" value="${tmp}"/>
					            <c:set var="totalQuantity" value="${totalQuantity + item.quantity2}"/>
					            <c:set var="totalBox" value="${totalBox + item.box_num}"/>
					            <c:set var="totalPerBox" value="${totalPerBox + item.per_box_num}"/>
					            <c:set var="totalMoney" value="${totalMoney + item.money}"/>
					            <%-- 
					        ...
					        <tr class="error">
					        	...
					        	<td class="center"><fmt:formatNumber value="${totalStockCnt}" pattern="#,###" /></td>
					        	<td class="center" colspan="10"></td>
					        	<td class="center"><fmt:formatNumber value="${totalBox}" pattern="#,###" /></td>
					        	<td class="center"><fmt:formatNumber value="${totalPerBox}" pattern="#,###" /></td>
					        	<td></td>
					        	<td class="center"><fmt:formatNumber value="${totalQuantity}" pattern="#,###.00" /></td>
					        	<td class="center"><fmt:formatNumber value="${totalMoney}" pattern="#,###.00" /></td>
					        	<td></td>
					        	<td></td>
					        </tr>
						</c:when>
						<c:otherwise>
							<tr>
								<td scope="col" colspan="22" class="center bin">데이터가 없습니다.</td>
							</tr>
						</c:otherwise>
					</c:choose>
                    </tbody>
