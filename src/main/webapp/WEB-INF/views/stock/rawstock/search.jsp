...
<div class="row-fluid">
	...
	<div class="block">
		<div class="navbar navbar-inner block-header">
			<div class="muted pull-left">原材料出入库</div>
		</div>
		<div class="block-content collapse in">
			<div class="span12">
				<div class="alert alert-block">
					<form method="post" class="form-horizontal" action="" id="searchForm" onsubmit="return paging.submit(); return false;">
						<%-- 검색조건 --%>
						<fieldset>
								...
								<div class="controls">
									<select id="selectStockGubun" name="stockGubun" class="" title="구분">
										...
										<c:forEach items="${stockGubuns}" var="item">
											<option value="${item.code}" ${item.code==pageParam.stockGubun ? 'selected="selected"' : ''}>${item.name}</option>
										</c:forEach>
									</select>
								</div>
							</div>
							...
							<div class="control-group">
								...
								<div class="controls">
									<select id="selectKind" name="kind" class="selectRawStock" title="종류">
										<option value="">全部</option>
										<c:forEach items="${kinds}" var="item">
											<option value="${item.kind}" ${item.kind==pageParam.kind ? 'selected="selected"' : ''}>${item.kind}</option>
										</c:forEach>
									</select>
								</div>
							</div>
							<div class="control-group">
								...
								<div class="controls">
									<select id="selectProductName" name="productName" class="selectRawStock" title="품명">
										<option value="">全部</option>
										<c:forEach items="${productNames}" var="item">
											<option value="${item.code}" ${item.code==pageParam.productName ? 'selected="selected"' : ''}>${item.code}</option>
										</c:forEach>
									</select>
								</div>
							</div>
							<div class="control-group">
								...
								<div class="controls">
									<select id="selectClient" name="client" class="selectRawStock" title="입고처">
										<option value="">全部</option>
										<c:forEach items="${clients}" var="item">
											<option value="${item.code}" ${item.code==pageParam.client ? 'selected="selected"' : ''}>${item.code}</option>
										</c:forEach>
									</select>
								</div>
							</div>
							<div class="control-group">
								...
								<div class="controls">
									<input id="loftNumber" name="loftNumber" class="input-large number" title="로트번호" value="${pageParam.loftNumber}" />
								</div>
							</div>
							<div class="control-group btn-group">
								<input type="hidden" name="pageCut" value="10"/>
								<input type="hidden" name="pageNum" value="${pageParam.pageNum}"/>
								<button type="submit" class="btn btn-primary" id="rawStockSearch">檢索</button>
							</div>
						</fieldset>
						<%--//검색조건 --%>
					</form>
						...
						<c:choose>
							<c:when test="${!empty list}">
								...
								<c:forEach items="${list}" var="item" varStatus="loop">
									<tr>
										<td class="center">
											<span>${loopIndex+loop.count}</span>
										</td>
										<td class="center">
											<c:choose>
												<c:when test="${item.isCheck=='OK' && userSession.id != 'bogoadmin'}">
													${item.created}
												</c:when>
												<c:otherwise>
													<a href="javascript:rawStockEdit('${item.idx}');">${item.created}</a>
												</c:otherwise>
											</c:choose>
										</td>
										...
										<td class="center" id="${item.idx}">
											<c:choose>
												<c:when test="${item.isCheck=='OK'}">
													${item.isCheck}
												</c:when>
												<c:otherwise>
													<a href="javascript:isCheckOk('${item.idx}');" class="isCheckBtn" >${item.isCheck}</a>
												</c:otherwise>
											</c:choose>
										</td>
									</tr>
									<c:set var="weightSum" value="${weightSum + item.weight}"/>
									<c:set var="realPriceSum" value="${realPriceSum + item.realPrice}"/>
								</c:forEach>
						        <tr class="error">
					                <td class="center" colspan="10">合计</td>
					                <td class="center"><fmt:formatNumber value="${weightSum}" pattern="#,###" /></td>
					                <td class="center"></td>
					                <td class="center"><fmt:formatNumber value="${realPriceSum}" pattern="#,###.00" /></td>
					                <td class="center"></td>
					                <td class="center"></td>
					        	</tr>
							</c:when>
				...
				<form id="submitForm" class="form-horizontal" onsubmit="return false;">
					<fieldset>
						...
						<div class="control-group">
							...
							<div class="controls">
								<input class="input-small date" title="입출고날짜" id="datePop" name="inputdate" type="text" >
							</div>
						</div>
						<div class="control-group">
							<label class="control-label" for="inputError">결산월</label>
						...
						<div class="control-group">
							<label class="control-label" for="workType0">区分</label>
							<div class="controls">
								<select id="selectStockGubunPop" name="rawGubun" class="" title="구분">
									<option value="">全部</option>
									<c:forEach items="${stockGubuns}" var="item">
										<option value="${item.code}" >${item.name}</option>
									</c:forEach>
						...
						<div class="control-group">
							<label class="control-label" for="inputError">种类</label>
							<div class="controls">
								<select id="selectKindPop" name="kind" class="selectRawStockPop" title="종류">
									<option value="">全部</option>
									<c:forEach items="${kinds}" var="item">
						...
						<div class="control-group">
							<label class="control-label" for="inputError">品名</label>
							<div class="controls">
								<select id="selectProductNamePop" name="productName" class="selectRawStockPop" title="품명">
									<option value="">全部</option>
									<c:forEach items="${productNames}" var="item">
						...
						<div class="control-group">
							<label class="control-label" for="inputError">入库社</label>
							<div class="controls">
								<select id="selectClientPop" name="client" class="selectRawStockPop" title="입고처">
									<option value="">全部</option>
									<c:forEach items="${clients}" var="item">
										<option value="${item.code}" >${item.code}</option>
									</c:forEach>
						...
						<div class="control-group">
							<label class="control-label" for="inputError">批号</label>
							<div class="controls">
								<input id="loftNumberPop" name="loftNumber" class="input-large number" title="로트번호" />
							</div>
						</div>
						<div class="control-group">
						...
						<div class="control-group">
							...
							<div class="controls">
								<input class="input-xlarge focused" id="weightPop" name="weight" type="number" title="중량" placeholder="중량" >
							</div>
						</div>
						<div class="control-group">
							<label class="control-label" for="inputError">备注</label>
							<div class="controls">
								<input id="bigoPop" name="bigo" class="input-large number skip" title="비고" />
							</div>
						</div>
						<div class="control-group">
							<label class="control-label" for="inputError">单价</label>
							<div class="controls">
								<input id="pricePop" name="price"  style="text-align: right;" title="단가" readonly />
							</div>
						</div>
					</fieldset>
...
$(function(){
	...
	$('#selectKindPop').change(function()
	{
	    ...
	    if(kind){
	        $.ajax({
	            ...
	            if(data.list){
	            	var $selectProductName = $('#selectProductNamePop').find('option:not(:eq(0))').remove().end();
                    $.each(data.list, function(i, item){
                        $selectProductName.append($('<option/>').val(item.code).text(item.name));
                    });
                    $selectProductName.prop('disabled', false);
	...
	<%-- 팝업 - 품목 셀렉트 변경 --%>
	$('#selectProductNamePop').change(function()
	{
        ...
        if(kind &&  productName){
            $.ajax({
                ...
                type: 'POST',
                data: {kind: kind, productName : productName}
            }).success(function( data ) {
                if(data.list){
	...
	<%-- 팝업 - 입고처 셀렉트 변경 --%>
	$('#selectClientPop').change(function()
	{
        var kind = $('#selectKindPop').val();
        ...
        if(kind &&  productName && client){
            $.ajax({
                ...
                type: 'POST',
                data: {kind: kind, productName : productName,client:client}
            }).success(function( data ) {
                if(data.obj){
	...
	$('#datePop').change(function(){
		...
		$('#settlementMonthPop').find('[name=settlementMonth]').val(day).end()
	});
...
function rawStockUpdate(rowData)
{
	if(rowData)
	{
		...
		$('#rawStockModal').find('[name=eventType]').val('U').end()
			...
			.find('#selectStockGubunPop').val(rowData.rawGubunCd).trigger('change').end()
			.find('#selectKindPop').val(rowData.kind).trigger('change').end()
            ...
            .find('#pNumberPop').val(rowData.pNumber).end().modal('show');
	}
}
...
function isCheckOk(idx){
	...
	$.ajax({
		...
		type: 'POST',
		data: {idx: idx, eventType: "U", isCheck : "OK" }
	}).success(function( data ) {
		 alert("확인처리에 성공하였습니다." );
		 if(data.code == '0000'){	
				$("#"+idx).text("OK");
